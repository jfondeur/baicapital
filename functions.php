...
function canablue_footer_script()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_register_script('jquerycustom', get_template_directory_uri() . '/js//lib/jquery.min.js', array('jquery'), '3.2.1'); // Custom scripts
...
{
    global $post;
    return '... <a class="view-article d-block mt-2" href="' . get_permalink($post->ID) . '">' . __('View Article', 'html5blank') . '</a>';
}
...
add_action('init', 'create_projects_custom_post'); // Add our HTML5 Blank Custom Post Type
...
function create_projects_custom_post()
{
    ...
    register_post_type('projects', // Register Custom Post Type
        array(
        'labels' => array(
            'name' => __('Project', 'html5blank'), // Rename these to suit
